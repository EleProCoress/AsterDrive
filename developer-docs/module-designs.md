...
### 为什么 `background_task` 表既是队列也是历史表

当前没有额外引入“队列表 + 历史表”双模型，而是让 `background_task` 同时承担：
...
- `Fallback`：`storage_policy_temp_cleanup`、`trash_purge_all` 和系统运行记录的兜底 lane，并发上限来自 `background_task_max_concurrency`
...
迁移任务可能很长，不能只靠 `background_task` 一行记录表示进度。当前实现会为每个迁移任务写一条 `storage_migration_checkpoint`：
