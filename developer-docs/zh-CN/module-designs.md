...
每种业务任务都先在 `src/services/task/spec/` 实现 `BackgroundTaskSpec`，再由 `src/services/task/registry.rs` 注册。这个 spec 是任务类型的单一契约，集中声明：

- `BackgroundTaskKind`
- 强类型 payload / result 编解码
- 初始 steps
- lane
- max attempts 和 retry class
- process 入口

创建任务应走 typed create helper，不要直接拼 `background_task::ActiveModel` 或手写 `payload_json` / `result_json`。否则 dispatcher、展示层、重试策略和创建路径很容易各自维护一份不一致的 `kind` 分支。
...
- `OfflineDownload`：`offline_download`，并发上限来自 `offline_download_max_concurrency`
...
archive 和 thumbnail lane 会在单轮 dispatch 里快速继续捞下一批，避免大量同类任务只靠下一次周期 tick 慢慢推进。OfflineDownload 单独限流，避免慢速外部下载占用归档、缩略图或维护任务预算。StorageMigration lane 独立限流，避免大规模策略迁移占满其他业务 lane。Fallback lane 更保守，避免维护型任务抢走太多资源。
