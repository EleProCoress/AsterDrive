---
description: AsterDrive external authentication guide covering OpenID Connect, generic OAuth2, Logto, GitHub, Google, Microsoft, account-linking policy, and troubleshooting.
---
...
AsterDrive currently supports five provider kinds:
...
Prefer `oidc` when the provider supports standard OIDC. Use dedicated providers for GitHub, Google, and Microsoft. Use `generic_oauth2` only when the provider does not have complete OIDC discovery or you intentionally need OAuth2 UserInfo mapping.
...
`{kind}` is `oidc`, `generic_oauth2`, `github`, `google`, or `microsoft`, and `{provider}` is the provider key generated by the server.
...
| Issuer URL | Required for OIDC; optional for Generic OAuth2. For Generic OAuth2, it becomes the identity namespace if set. GitHub uses fixed endpoints; Google is fixed to `https://accounts.google.com`; Microsoft derives it from the tenant |
| Authorization URL | Required for Generic OAuth2; discovered for OIDC; fixed for GitHub; discovered from Google / Microsoft fixed discovery |
| Token URL | Required for Generic OAuth2; discovered for OIDC; fixed for GitHub; discovered from Google / Microsoft fixed discovery |
| UserInfo URL | Required for Generic OAuth2; OIDC currently primarily uses ID Token claims; GitHub reads `https://api.github.com/user`; Google / Microsoft currently primarily use ID Token claims |
