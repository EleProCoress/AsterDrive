...
| 对象存储直传 | 浏览器直接把文件传到 S3 / MinIO / 腾讯云 COS，服务端只负责签名和完成确认。 | [上传与大文件](./upload-modes) |
| 服务端转发 | 浏览器先把文件传给 AsterDrive，再由服务端写到 S3 / MinIO / 腾讯云 COS。 | [上传与大文件](./upload-modes) |
...
| WOPI | Office 在线预览/编辑协议。AsterDrive 提供文件接口，OnlyOffice / Collabora 等服务负责打开文件。 | [文件编辑](./editing) |
| 预览应用 | 后台配置的文件打开方式，可以是内置预览、外部 URL 模板或 WOPI 应用。 | [系统设置](/config/runtime) |
| 审计日志 | 记录关键用户和管理员动作，用于排查、追踪和日常检查。 | [管理后台](./admin-console) |
