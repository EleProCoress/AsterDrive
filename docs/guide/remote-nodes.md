...
当前内部远程存储协议版本是 `v2`。主控测试连接和绑定远程策略时，会读取 follower 暴露的能力信息，包括协议版本范围、服务端版本、对象读写能力、Range 能力、compose 能力、metadata 能力，以及浏览器直传所需的 CORS 契约。
...
测试连接通过后，主控会在远程节点详情里展示能力摘要。至少要看到协议版本兼容当前主控，才适合继续创建 remote 存储策略。当前主控要求 follower 至少支持内部协议 `v2`。
