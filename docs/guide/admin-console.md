...
| 接入 OIDC / 通用 OAuth2 / SSO 登录 | `管理 -> 外部认证` | 本页 [外部认证](#外部认证) |
...
`外部认证` 页面负责管理登录页上的外部身份提供商。当前内置支持 OpenID Connect 和通用 OAuth2；完整接入说明见 [外部认证](/config/external-auth)。

你可以在这里：

- 创建外部认证提供商
- 选择 OpenID Connect 或通用 OAuth2
