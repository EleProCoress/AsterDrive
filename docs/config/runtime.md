...
| 后台任务、缩略图、ZIP 预览、回收站保留策略异常 | 运行与调度 / 存储与保留 | 再看 [运维 CLI](/deployment/ops-cli) |
| 分享页视频/音频播放链接过期太快或太慢 | 运行与调度 | 再看 [分享与公开访问](/guide/sharing) |
| WebDAV 总开关、系统文件拦截或连接异常 | WebDAV | 再看 [WebDAV](/config/webdav) |
| 要查谁改了什么 | 审计日志 | 再看 [管理后台](/guide/admin-console#审计日志) |
...
| 开启或限制 ZIP 压缩包只读预览 | `存储与保留 -> 压缩包预览` |
| 接入 OIDC / SSO 登录提供商 | `管理 -> 外部认证` |
...
| ZIP 压缩包预览用户侧开关 | 关闭 |
| ZIP 压缩包预览分享侧开关 | 关闭 |
| ZIP 压缩包预览源文件大小上限 | `64 MiB` |
| ZIP 压缩包预览条目数量上限 | `2000` |
| ZIP 压缩包预览 manifest 大小上限 | `64 KiB` |
| ZIP 压缩包预览扫描耗时上限 | `30` 秒 |
| 缩略图源文件大小上限 | `64 MiB` |
...
| ZIP 压缩包预览开关和限制 | 之后新请求和新建的 `archive_preview_generate` 任务按新规则 |
