...
| 文件记录 | 文件名冲突、版本、Blob 引用、锁状态和属性 | [文件编辑](/guide/editing)、[架构概览](/guide/architecture) |
...
| 模块 | 负责内容 |
| --- | --- |
| `workspace_scope_service`、`workspace_models` | 个人空间和团队空间作用域 |
| `file_service`、`folder_service` | 文件、文件夹、路径、列表和权限校验 |
| `workspace_storage_core`、`workspace_storage_service` | 文件记录、Blob、配额和策略落账 |
| `team_service` | 团队、成员、角色和归档 |
| `share_service`、`share_public` routes | 分享创建、公开访问和分享范围 |
| `trash_service`、`version_service`、`lock_service` | 回收站、版本和文件锁 |
| `property_service` | 文件/文件夹扩展属性 |
