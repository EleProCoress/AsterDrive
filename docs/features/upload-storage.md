...
| 模块 | 负责内容 |
| --- | --- |
| `files::upload` | 上传会话、分片、进度、状态转换 |
| `workspace::storage_core` | Blob 去重、文件记录、配额、策略选择和最终落账 |
| `storage_policy::policy` | 存储策略、策略组和规则 |
...
| `remote::remote_node`、`remote::storage_target` | 远程节点和远程存储目标 |
| `task::storage_migration` | 存储迁移任务 |
...
- 远程节点策略失败：先看节点启用状态、传输方式、策略绑定的远程存储目标、默认目标和协议能力。
