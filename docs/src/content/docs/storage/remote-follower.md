...
:::tip[这一篇覆盖什么]
这一篇讲怎么把已经接入的从节点作为存储策略后端使用：创建远程存储目标、创建 `remote` 存储策略、配置策略组规则、绑定用户或团队，并验收真实上传下载。
:::
...
```mermaid
flowchart TD
  Node["远程节点记录"] --> Target["远程存储目标"]
  Target --> Policy["remote 存储策略"]
  Policy --> Rule["策略组规则"]
...
| 远程存储目标 | 从节点收到对象后写到本地还是 S3 | `管理 -> 远程节点 -> 节点详情` |
...
```mermaid
flowchart LR
  Browser["浏览器"] --> Primary["主控节点"] --> Follower["从节点"] --> Target["远程存储目标"]
```
...
```mermaid
flowchart LR
  Target["远程存储目标"] --> Follower["从节点"] --> Primary["主控节点"] --> Browser["浏览器"]
```
...
:::caution[Tailscale / VPN 地址不是公网地址]
