...
如果你需要浏览器直传，优先看 [S3 / MinIO / R2](/storage/s3-minio-r2/)、[Azure Blob Storage](/storage/azure-blob/) 或 [腾讯云 COS](/storage/tencent-cos/)。SFTP 的上传和下载都经过 AsterDrive 服务端。
...
| SSH 用户名 | `asterdrive` | 用于登录 SFTP 服务器的账号 |
| SSH 密码 | `********` | 对应账号的密码 |
...
:::caution[不要跳过主机密钥确认]
如果不校验 SSH 主机密钥，攻击者可以伪装成 SFTP 服务器并获取密码。AsterDrive 会保存确认后的指纹，后续连接必须匹配；服务器重装或更换 SSH 主机密钥后，需要重新确认并更新。
:::

## 4. 连接测试失败怎么查

连接测试失败时，后台会显示可用于排查的原因。常见排查顺序：
...
:::caution[已经写入文件的 SFTP 策略，不要直接改这些字段]
