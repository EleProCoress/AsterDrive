...
| 模块 | 负责内容 |
| --- | --- |
| `auth::local` | 注册、密码、会话、邮箱验证和登录主流程 |
| `auth::mfa` | MFA 登录流程、TOTP、恢复码、邮箱验证码 |
| `auth::passkey` | Passkey 注册、认证和凭证管理 |
| `auth::external` | 外部 provider、登录流程、身份解析、账号绑定 |
| `webdav::account` | WebDAV 专用账号和访问范围 |
