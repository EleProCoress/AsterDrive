---
title: "Docker Follower Node Deployment"
---
...
In the current version, the follower's object destination is pushed by the primary node from the follower node details page.
In other words, Docker bootstrap only binds the primary and follower identities. Whether objects are ultimately written to a follower local directory or S3 is configured later in the primary admin panel by creating a **remote storage target**.
...
Remote storage targets are not passed in bootstrap ENV.
The current flow is: the follower enrolls first, then the primary node pushes remote storage targets through the follower API. The entry is `Admin -> Follower Nodes`, which is better for later review, modification, and troubleshooting.
...
Click "Test Connection". Direct nodes access `base_url`; reverse-tunnel nodes use the outbound channel maintained by the follower, which may need a few dozen seconds after startup before it becomes online. After the test passes, open the follower node details and create a **default remote storage target**.

When the connection test passes, the primary also reads the follower's internal storage protocol capabilities. The current primary uses `v5` and supports followers down to `v4`. If the declared protocol ranges do not overlap, upgrade the primary or follower before creating remote policies.

For the first remote storage target, choose:

- Driver: `local`
- Base path: a relative path such as `default`
- Check "Set as default remote storage target"

The path for a local remote storage target is restricted under the follower's `server.follower.remote_storage_target_local_root`.
If you want the follower to write to S3 / MinIO instead, create an `s3` remote storage target here too, not through bootstrap ENV.

After the remote storage target is applied, go to:
...
After confirming the follower is ready, the primary connection test passes, and the default remote storage target has been applied, remove these ENV values from Compose:
