...
```text
管理 -> 系统设置 -> 文件处理 -> 媒体处理 -> 图片预览策略
```

`优先原图` 适合希望尽量看到原始文件的站点；`优先中等预览` 适合大照片较多、带宽较敏感，或者希望先快速打开预览再按需查看原图的站点。无论选哪一种，浏览器不支持当前原图格式时都会回退到后端预览图。

图片查看器支持全屏、缩放、拖动、旋转，以及在当前列表、文件夹或分享范围内切换上一张 / 下一张。后端预览图第一次缺失时会创建 `image_preview_generate` 后台任务；任务完成前，页面可能会先显示加载或回退状态，后续再打开会复用缓存。
...
- 保存成功后生成历史版本
- 文件编辑期间会出现锁；部分客户端或协议场景允许同一资源上存在多把共享锁
- 有效锁存在时，冲突的覆盖、移动或删除会被拒绝
- 过期锁会被后台清理，也可以由管理员手动清理
