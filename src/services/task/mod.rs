...
//! payload/result/列表 DTO 统一从 `task::types` 导入；各任务族的创建入口
//! 从对应子模块导入，例如 `task::archive`、`task::thumbnail`、
//! `task::storage_migration`。不要把新的任务类型或内部 helper 继续 re-export
//! 到根模块。
...
impl<S: BackgroundTaskSpec> TypedTaskCreate<S> {
    ...
    pub(in crate::services::task) fn new(
    ...
    pub(in crate::services::task) fn creator_user_id(
        mut self,
        creator_user_id: Option<i64>,
    ) -> Self {
    ...
    pub(in crate::services::task) fn next_run_at(
        mut self,
        next_run_at: chrono::DateTime<Utc>,
    ) -> Self {
    ...
    pub(in crate::services::task) fn progress(mut self, current: i64, total: i64) -> Self {
        self.progress_current = current;
        self.progress_total = total;
        self
    }

    pub(in crate::services::task) fn status_text(mut self, status_text: String) -> Self {
    ...
    pub(in crate::services::task) fn result(mut self, result: &S::Result) -> Result<Self> {
        self.result_json = Some(spec::serialize_result::<S>(result)?);
        Ok(self)
    }

    pub(in crate::services::task) fn without_steps(mut self) -> Self {
        self.include_steps = false;
        self
    }

    pub(in crate::services::task) fn started_at(
        mut self,
    ...
    pub(in crate::services::task) fn finished_at(
        mut self,
    ...
    pub(in crate::services::task) fn failure_can_retry(
        mut self,
        failure_can_retry: Option<bool>,
    ) -> Self {
...
pub(in crate::services::task) async fn insert_typed_task_record<
    C: ConnectionTrait,
...
mod tests {
    ...
    fn typed_task_create_truncates_status_text_and_last_error() {
        ...
        let payload = crate::services::task::types::ThumbnailGenerateTaskPayload {
            blob_id: 42,
