...
impl OfflineDownloadRateLimiter {
    pub(in crate::services::task) fn new(max_bytes_per_sec: Option<u64>) -> Option<Self> {
    ...
    pub(in crate::services::task) async fn throttle(
        limiter: Option<&Self>,
        chunk_len: usize,
...
pub(in crate::services::task) fn declared_content_length(
...
pub(in crate::services::task) fn verify_expected_sha256(
    expected: Option<&str>,
    actual: &str,
) -> Result<()> {
...
pub(in crate::services::task) fn ensure_download_size_allowed(
    size: i64,
    max_bytes: i64,
) -> Result<()> {
    if size > max_bytes {
